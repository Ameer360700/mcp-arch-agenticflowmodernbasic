...
export const checkRectangleTool = {
  profile: {
    name: "check_rectangle",
    description: "Checks if a rectangle exists on the canvas. Provide x, y, width, height.",
    parameters: {
      type: "object",
      properties: {
  ...
  execute: async (args) => {
    console.log("Tool Execution Started: check_rectangle", JSON.stringify(args));
    ...
    if (!fs.existsSync(filePath)) {
      return {
        status: "success",
        exists: false,
        message: "The canvas file does not exist yet, so the rectangle is not drawn."
      };
    }
    ...
    try {
      ...
      // Sample points along all 4 edges of the rectangle
      const points = [];
      const steps = 10;
      ...
      return {
        ...
        exists,
        message: exists
          ? `Rectangle at (${x}, ${y}) with size ${width}x${height} was successfully detected.`
          : `Rectangle at (${x}, ${y}) with size ${width}x${height} was not found.`
      };
